...
$sq1 = mysql_query("select sum(by_nom) jum from bayar_detail where by_nodaf = '".$_SESSION[ps_nodaf]."' and by_status='verified'");
	...
	if ($arai1['jum'] < $jumbiaya){$keterangan= "Mohon maaf, saat ini anda belum bisa mendapatkan pasword ujian online karna alasan biaya pendidikan yang belum terselesaikan. Silakan hubungi bagian keuangan untuk prosses lebih lanjut. Terima kasih";} 
	...
	$sqpas = mysql_query("select * from tbuser_ujian where nama ='$_SESSION[ps_nama]'");
	$araipas = mysql_fetch_array($sqpas);
	$password=$araipas['pwd'];
	...
	if ($arai1['jum'] < $jumbiaya){$keterangan= "Mohon maaf, saat ini anda belum bisa mendapatkan pasword ujian online karna alasan biaya pendidikan yang belum terselesaikan. Silakan hubungi bagian keuangan untuk prosses lebih lanjut. Terima kasih";} 
	else {  $keterangan=" Selamat, anda mendapatkan pasword ujian online karna anda telah memenuhi persyaratan pembayaran angsuran keempat. Silakan gunakan pasword ini untuk mengikuti ujian online.";
	
	$sqpas = mysql_query("select * from tbuser_ujian where nama ='$_SESSION[ps_nama]'");
	$araipas = mysql_fetch_array($sqpas);
	$password=$araipas['pwd'];
	if($password == ""){  echo "Mohon maaf, terjadi kesalahan sistem!"; $keterangan="";} } ?>
	<h3> PASSWORD UJIAN KEEMPAT : <strong><font color=green><?php 	echo $password;?></font></strong> </h3>
	<? echo $keterangan."<br><br><br>";
