...
if ($_GET[tanggal_transaksi]==""){ $_SESSION[tanggal_transaksi]=date("d/m/Y");} else{$_SESSION[tanggal_transaksi]=$_GET[tanggal_transaksi];}
...
<link href="style.css" rel="stylesheet" type="text/css" media="screen" />
	...
	var koderek = document.forms["postform"]["kode_rekening"].value;
    if (koderek == null || koderek == "") {
        alert("Mohon Pilih Kode Penyeimbang Sisi Debet Dengan Kode Angka 5!");
        return false;
    }
	
	var uraiantransaksi = document.forms["postform"]["keterangan_transaksi"].value;
    if (uraiantransaksi == null || uraiantransaksi == "") {
        alert("Anda belum mengisi uraian transaksi!");
        return false;
    }
	
	var jum = document.forms["postform"]["jumlah_dk"].value;
    if (jum == null || jum == "") {
	...
	<div class="post">
		<div class="entry">
			<h2 align="center"><strong>Jurnal Kas Keluar <? echo $_SESSION[unit];?></strong></h2>
			Form ini digunakan oleh cabang untuk menginput laporan pengeluaran keuangan cabang. <br>Silakan pilih kode rekening perkiraan sisi debet dengan awalan angka <strong>5</strong> sebagai penyeimbang sisi kredit kas cabang. <br> Masukkan laporan pengeluaran setiap hari karna setelah anda menekan tombol simpan pada form ini, <br>aplikasi akan mengirimkan sms informasi pengeluaran keuangan cabang ke accounting pusat.
			<p>
			<?php 
			...
			<form action="?page=./transaksi/umum" method="post" name="postform" id="postform" onsubmit="return validateForm()">
			  <table width="80%" border="0">
				...
				<tr style="visibility:hidden;">
				  ...
				  <div id="suggest">
					   ...
					   value="<? $querya=mysql_query("select kode_rekening,nama_rekening from tabel_master where nama_rekening like 'Kas Cabang $_SESSION[unit]'");
			while($rowa=mysql_fetch_array($querya)){ $koderekeningx=$rowa[0]; $namarekeningx=$rowa[1];} echo $koderekeningx; ?> "/> 
                  ...
                  <td width="203" align="left"><input type="text" disabled="disabled" name="nama_rekeningkredit" onBlur="fillkredit();" id="countrykredit"  value=<? echo $namarekeningx;?>/></td>
                </tr>
			...
			if(isset($_POST['simpan'])){
				...
				//kirim sms ke pak iwan & pak paulus
