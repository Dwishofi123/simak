...
<div class="modal modal-draggable modal_default_1" id=cari_anggaran tabindex=-1 role=dialog aria-labelledby=myModalLabel aria-hidden=true>
 ...
 <h4 class=modal-title>Cari Absensi Siswa</h4> </div>
...
<div style="visibility:hidden;"><input type=text name="page" value="absensi_siswa"></input></div>
 ...
 <div class="head bg-default bg-light"> <h2><br>Data Absensi Siswa <?  if ($_POST[cari_tanggal_1]<>''){echo $_POST[cari_tanggal_1]." hingga ".$_POST[cari_tanggal_2];}?>
</h2> </div> 
	...
	<ul class="dropdown-menu" role="menu">   
		 <li ><a href=# data-toggle=dropdown>Cari Absensi</a>
			 <ul class=dropdown-submenu> 
			   		<li><a href="absensi_siswa_cari.php">Seluruh Siswa PSPP Penerbangan </a></li> 
					<li><a href="absensi_siswa_cari.php?unit=PSPP Lampung">Siswa PSPP Lampung</a>
							<ul class=dropdown-submenu> 
							<? $query = mysql_query("SELECT absensi.id,absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan FROM absensi where absensi.id<>'' and status_user='Siswa'  $filter_cari_nama $_SESSION[cari_tanggal] and unit='PSPP Lampung' order by absensi.tanggal asc limit 250");  $a=0; while($b=mysql_fetch_row($query)) { $a=$a+1;?>
							<li><a href="absensi_siswa_cari.php?cari_nama=<? echo $b[1];?>"><? echo $a.".".$b[1];?></a></li> <? }?>
							</ul>
					</li> 
					<li><a href="absensi_siswa_cari.php?unit=PSPP Jakarta">Siswa PSPP Jakarta</a>
							<ul class=dropdown-submenu> 
							<? $query = mysql_query("SELECT absensi.id,absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan FROM absensi where absensi.id<>'' and status_user='Siswa'  $filter_cari_nama $_SESSION[cari_tanggal] and unit='PSPP Jakarta' order by absensi.tanggal asc limit 250");  $a=0; while($b=mysql_fetch_row($query)) { $a=$a+1;?>
							<li><a href="absensi_siswa_cari.php?&cari_nama=<? echo $b[1];?>"><? echo $a.".".$b[1];?></a></li> <? }?>
							</ul>
					</li> 
					<li><a href="absensi_siswa_cari.php?unit=PSPP Jogja">Siswa PSPP Yogyakarta</a>
							<ul class=dropdown-submenu> 
							<? $query = mysql_query("SELECT absensi.id,absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan FROM absensi where absensi.id<>'' and status_user='Siswa'  $filter_cari_nama $_SESSION[cari_tanggal] and unit='PSPP Jogja' order by absensi.tanggal asc limit 250");  $a=0; while($b=mysql_fetch_row($query)) { $a=$a+1;?>
							<li><a href="absensi_siswa_cari.php?cari_nama=<? echo $b[1];?>"><? echo $a.".".$b[1];?></a></li> <? }?>
							</ul>
					</li> 
					<li><a href="absensi_siswa_cari.php?unit=PSPP Makassar">Siswa PSPP Makassar</a>
							<ul class=dropdown-submenu> 
							<? $query = mysql_query("SELECT absensi.id,absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan FROM absensi where absensi.id<>'' and status_user='Siswa'  $filter_cari_nama $_SESSION[cari_tanggal] and unit='PSPP Makassar' order by absensi.tanggal asc limit 250");  $a=0; while($b=mysql_fetch_row($query)) { $a=$a+1;?>
							<li><a href="absensi_siswa_cari.php?cari_nama=<? echo $b[1];?>"><? echo $a.".".$b[1];?></a></li> <? }?>
							</ul>
					</li> 
					<li><a href="absensi_siswa_cari.php?filter=instruktur">Instruktur</a></li> 
			 </ul>
	</li>
...
<div class="tab-pane" id="tab13"> <div id="divToPrint2"><p>
 ...
 <? $querya = mysql_query("SELECT  distinct absensi.tanggal FROM absensi where absensi.id<>'' and status_user='Siswa'  $filter_cari_nama $_SESSION[cari_tanggal] $filter_cari_kampus order by absensi.tanggal asc limit 250"); while($u=mysql_fetch_row($querya)) { $tgggl=$u[0];?>  
 ...
 <tr> <th width="30">No</th> <th>Nama Siswa</th> <th >Jam Masuk</th> <th >Jam Keluar </th><th>Keterangan</th>  <th >Unit Kerja</th> </tr> 
</thead> <tbody> 
<?$a=0; 
 if($_SESSION[hak_akses_absensi_pegawai]<>"true"){
 $query = mysql_query("SELECT absensi.id, absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan,absensi.unit FROM absensi where absensi.id_user='$_SESSION[id_login]' and status_user='Siswa'  $filter_cari_tanggal $filter_instruktur order by absensi.tanggal asc limit 250");
 }else{
$query = mysql_query("SELECT absensi.id,absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan,absensi.unit FROM absensi where absensi.tanggal='$tgggl' $filter_cari_nama $filter_cari_kampus and status_user='Siswa'  $filter_instruktur order by absensi.tanggal asc limit 250");}
