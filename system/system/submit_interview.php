...
$nomorinter = $iki + 1;
$kueri_interview = mysql_query("insert into interview (id,interview_by,tgl_interview,kampus,angkatan,id_siswa,nama,tempat_lahir,tgl_lahir,email,ukuran_baju,ukuran_sepatu,asal_sekolah,kota_sekolah,jilbab,dukungan_ortu,biaya_pendidikan,nama_ortu,alamat,kode_pos,pengetahuan,etika,kemampuan_khusus,speaking_ability,communication_skills,performance,berat_badan,tinggi_badan,dijanjikan_kerja,status,diterima_jurusan,marketing,jalur_interview,catatan_interview,tempat_interview) values ('$nomorinter','$_SESSION[nama_login]','$tgl_inteview','$diterima_kampus','$diterima_angkatan','$id_siswa','$nama','$tempat_lahir','$tgl_lahir','$email','$ukuran_baju','$ukuran_sepatu','$asal_sekolah','$kota_sekolah','$jilbab','$dukungan_ortu','$biaya_pendidikan','$nama_ortu','$alamat','$kode_pos','$pengetahuan','$etika','$kemampuan_khusus','$berbicara','$komunikasi','$performance','$berat_badan','$tinggi_badan','$dijanjikan','$status','$diterima_jurusan','$marketing','$jalur_interview','$catatan','$_SESSION[unit]')") or die("".mysql_error());
	IF($kueri_interview==true)
	{
		...
		mysql_query("update siswa set Nama = '$nama',tempat_lahir = '$tempat_lahir',tanggal_lahir = '$tgl_lahir',email = '$email',ukuran_baju = '$ukuran_baju',ukuran_sepatu = '$ukuran_sepatu',Asal_Sekolah = '$asal_sekolah',kota_sekolah = '$kota_sekolah',jilbab = '$jilbab',nama_ortu = '$nama_ortu',Alamat = '$alamat',berat_badan = '$berat_badan',tinggi_badan = '$tinggi_badan',Jurusan = '$jurusan_siswa' ,id_interview = '$nomorinter' , interview_by = '$_SESSION[nama_login]', tgl_interview = '$tgl_inteview', Tempat_Kuliah = '$diterima_kampus' where ID = '$id_siswa'");
		echo "<script>alert('Data Berhasil disimpan');window.location='form_interview_staff.php?id=$id&cetak=true&nomor_inter=$nomorinter&dukungan_ortu=$dukungan_ortu$ini';</script>";
	}
	else
	{
		echo "<script>alert('Data gagal disimpan');window.location='form_interview_staff.php?id=$id&dukungan_ortu=$dukungan_ortu$ini';</script>";
