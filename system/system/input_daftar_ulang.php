...
function PrintDiv13() {    
	...
	function getTable(id) {
		...
		jQuery.ajax({
			url: "system/input_daftar_ulang_table.php",
			data:'id='+id,
			type: "POST",
	...
	kelas = kelasValue.value;
		$('#tables').html("<img src='img/big_loading.gif'/><font color=white> <br>Loading... Mohon tunggu..</font>");
		jQuery.ajax({
			url: "system/input_daftar_ulang_table.php?kelas="+kelas,
			data:'kelas='+kelas,
			type: "POST",
			success:function(data){$('#tables').html(data);}
		});
	  ...
	  function DaftarUlang(id){
        $.colorbox({iframe:true, width:"50%", height:"80%", href: "system/submit_input_daftar_ulang.php?id="+id+"&status=daftarulang"});
      }
	  function Batal(id){
        $.colorbox({iframe:true, width:"30%", height:"20%", href: "system/submit_input_daftar_ulang.php?id="+id+"&status=batal"});
      }
	function BoxEdit(id){
	  ...
	  function CetakKwitansi(id){
        $.colorbox({iframe:true, width:"60%", height:"80%", href: "system/kwitansi_daftar_ulang.php?id="+id+"&status=cetak_kwitansi"});
      }
	  function MengundurkanDiri(id){
        $.colorbox({iframe:true, width:"30%", height:"20%", href: "system/submit_input_daftar_ulang.php?id="+id+"&status=mengundurkan_diri"});
      }
 ...
 <div class="head bg-default bg-light"> <h2>Input Daftar Ulang </h2> </div> 				 
