...
mysql_query("insert into tbl_log(datetime,uraian)value('$tanggal','$_SESSION[nama_login] membuka halaman absensi siswa')");
?>
...
function CariAbsensi() {
        ...
        $.colorbox({iframe:true, width:"95%", height:"95%", href: "system/absensi_siswa_cari.php"});
...
<div style="visibility:hidden;"><input type=text name="page" value="absensi_siswa"></input></div>
 ...
 <div class="head bg-default bg-light"> <h2>Data Absensi Siswa <?  echo $tanggal;?>
</h2> </div> 
...
<div class=btn-group>
   ...
   <a href="#refresh" onClick='getPage(38)' ><button data-loading-text="Loading..."  id="fat-btn" class="btn btn-default btn-clean tip"  data-original-title="Refresh"><span class=icon-refresh></span></button> </a>
   <a onclick="PrintDiv();" data-toggle=modal ><button class="btn btn-default btn-clean tip"  data-original-title="Print"> <span class=icon-print> </button></a>
	...
	<li><a href="#refresh" onClick='getPage(38)' >Refresh</a></li> 
...
<div class="tab-pane" id="tab13"> <div id="divToPrint"><p>
 ...
 <? $querya = mysql_query("SELECT  distinct absensi.tanggal FROM absensi where absensi.id<>'' and status_user='siswa' $filter_cari_nama $_SESSION[cari_tanggal] $filter_cari_kampus order by absensi.tanggal desc limit 250"); while($u=mysql_fetch_row($querya)) { $tgggl=$u[0];?>  
...
<?$a=0; 
 ...
 $query = mysql_query("SELECT absensi.id, absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan,absensi.unit FROM absensi where absensi.id_user='$_SESSION[id_login]' and status_user='Siswa'  $filter_cari_tanggal order by absensi.tanggal desc limit 250");
 }else{
$query = mysql_query("SELECT absensi.id,absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan,absensi.unit FROM absensi where absensi.tanggal='$tgggl' $filter_cari_nama and status_user='Siswa'  $filter_cari_kampus order by absensi.tanggal desc limit 250");}
