...
function CariAbsensi() {
        ...
        $.colorbox({iframe:true, width:"95%", height:"95%", href: "system/absensi_pegawai_cari.php"});
...
<div style="visibility:hidden;"><input type=text name="page" value="absensi_siswa"></input></div>
 ...
 <div class="head bg-default bg-light"> <h2>Data Absensi Pegawai <?  echo $tanggal;?>
</h2> </div> 
...
<? mysql_query("insert into tbl_log(datetime,uraian)value('$tanggal','$_SESSION[nama_login] membuka halaman absensi karyawan')");
...
<div class=btn-group>
   ...
   <a href="#refresh" onClick='getPage(25)' ><button data-loading-text="Loading..."  id="fat-btn" class="btn btn-default btn-clean tip"  data-original-title="Refresh"><span class=icon-refresh></span></button> </a>
   <a onclick="PrintDiv();" data-toggle=modal ><button class="btn btn-default btn-clean tip"  data-original-title="Print"> <span class=icon-print> </button></a>
	...
	<ul class="dropdown-menu" role="menu">   
		 <li ><a href=#cari_absensi onClick="CariAbsensi();"><font color="yellow">Cari Absensi</font></a>	</li>

	<li><a href="javascript:void(0);" id="export_13" data-toggle=modal>Export ke Excel</a></li>
	<li><a href="#refresh" onClick='getPage(25)' >Refresh</a></li> 
...
<div class="tab-pane" id="tab13"> <div id="divToPrint"><p>
 ...
 <? $querya = mysql_query("SELECT  distinct absensi.tanggal FROM absensi where absensi.id<>'' and status_user='Manajemen' $filter_cari_nama $_SESSION[cari_tanggal] $filter_cari_kampus order by absensi.tanggal desc limit 250"); 
 
 while($u=mysql_fetch_row($querya)) { $tgggl=$u[0];?>  
...
<?$a=0; 
 ...
 $query = mysql_query("SELECT absensi.id, absensi.id_user, absensi.tanggal, absensi.jam_masuk, absensi.jam_keluar, absensi.keterangan,absensi.unit FROM absensi where absensi.id_user='$_SESSION[id_login]' and status_user='Manajemen'  $filter_cari_tanggal order by absensi.tanggal desc limit 250");
 }else{
$query = mysql_query("SELECT * FROM absensi where absensi.tanggal='$tgggl' $filter_cari_nama and status_user='Manajemen'  $filter_cari_kampus order by absensi.tanggal desc limit 250");}

  while($b=mysql_fetch_array($query)) {
$a=$a+1;  ?>
<tr><td><? echo $a;?></td><td><? echo "$b[id_user]";?></td><td><? echo $b[jam_masuk];?></td><td><? echo $b[jam_keluar];?></td><td><? echo $b[keterangan];?></td><td><? echo $b[unit];?></td></tr>
<?} ?> 
