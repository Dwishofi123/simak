<? session_start();
include "konek.php"; $tanggal=date("d/m/Y");?>
<link href=acc_style.css rel=stylesheet type=text/css /> 
<?php 
	...
	<div class="post">
		<div class="entry">
			...
			if(isset($_POST['report'])){
				...
				$query_transaksi=mysql_query("select * from tabel_transaksi where tanggal_transaksi between '$tanggal1' and '$tanggal2' and id_admin='$_SESSION[id_login]' order by tanggal_transaksi asc");
				$total=mysql_fetch_array(mysql_query("select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_transaksi where tanggal_transaksi between '$tanggal1' and '$tanggal2' and id_admin='$_SESSION[id_login]' order by kode_rekening asc"));
	
			}else{
			
				$query_transaksi=mysql_query("select * from tabel_transaksi where id_admin='$_SESSION[id_login]' order by tanggal_transaksi asc");
				$total=mysql_fetch_array(mysql_query("select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_transaksi where id_admin='$_SESSION[id_login]' order by kode_rekening asc"));
