...
//if ($URL_REF_HOST <> "pspp-integrated.com" and $URL_REF_HOST<>"www.pspp-integrated.com")
if ($URL_REF_HOST <> "localhost" and $URL_REF_HOST<>"")
{
...
function isNumber(evt) {
        ...
        return true;
			...
			<?php 
			if($b['status_seleksi'] <> "Belum Diseleksi")
			{
				$url = "#";
				$keterangan = "<p style='color:red;'><strong><I>Pendaftaran anda sudah diprosses, biodata anda sudah terkunci secara otomatis oleh sistem. Apabila ada kesalahan data, silakan hubungi hotline kantor pusat PSPP Penerbangan untuk konfirmasi perubahan data anda melalui HOTLINE 0274-2802084. Terima kasih.</I></STRONG></P>";
			}
			else
			{
				$keterangan = "";
				$url = "submit_daftar.php?action=edit&id=$_GET[id]";
				
			}

			;?>
			<form  action="<?php echo $url;?>" class="form_pendaftaran" name="myForm" method="POST" onsubmit="return validateForm()">


			<?php 
			if($b['status_seleksi'] <> "Belum Diseleksi")
			{ echo "<font color=red>Pendaftaran anda sudah diprosses, biodata anda sudah terkunci secara otomatis oleh sistem. Apabila ada kesalahan data, silakan hubungi hotline kantor pusat PSPP Penerbangan untuk konfirmasi perubahan data anda melalui HOTLINE 0274-2802084. Terima kasih.</font>";}else{?>
			...
			{
					?>
					<input value=<?php if ($_GET['id']=="")	{	echo" Daftar ";	}	else	{	echo " Simpan ";	}	?> class="button" type="button" onclick="alert('Pendaftaran anda sudah diprosses, biodata anda sudah terkunci secara otomatis oleh sistem dan anda sudah tidak dapat merubahnya lagi. Apabila ada kesalahan data, silakan hubungi hotline kantor pusat PSPP Penerbangan untuk konfirmasi perubahan data anda melalui HOTLINE 0274-2802084. Terima kasih.');"></input>
				<?php
			}
			?>
		
			<input value=" Cetak Formulir Pendaftaran " class="button" type="button"  onclick="<?php if($b['tes_online'] == "sudah") { echo "javascript:void window.open('system/system/formulir_psb.php?id=$_SESSION[idpendaftaran]','1386361733215','width=840px,height=840px,toolbar=0,menubar=1,location=0,status=0,scrollbars=1,resizable=0,left=0,top=0');return false;"; } else { echo "alert('Maaf, Cetak Formulir Hanya Dapat Dilakukan Setelah Pendaftar Mengikuti Tes Online');"; } ;?>"></input>
			<BR><BR>
...
*/
   ...
   var asal_sekolah = document.forms["myForm"]["nama_sma"].value;
    if (asal_sekolah == null || asal_sekolah == "") {
        alert("Sepertinya anda belum mengisi asal sekolah anda, mohon periksa kembali. Untuk memilik nama sekolah, silakan pilih kabupaten/kota pendidikan terlebih dahulu.");
        return false;
    }	
